...
void loop() {
  ...
  if (currentMillis - previousMillis2 >= interval2) {
    ...
    if (index > 2)index = 0;
    delay(10);
    if (!emptyRow(index))sendCheck(index, 0xFE, 0x10, 0x34, 0x69,tempAlert);
...
void handleTransmitRequest() {
  ...
  switch (var) {
    ...
    case 1:
      ...
      break;
    case 0x02:
      Serial.println("threshold aanpassen");
      r = nextRead();
      Serial.print(r);
      Serial.println("========================================");
      setThreshold(r);
    ...
    case 0xFE:
      ...
      Serial.println("");
      Serial.println("");      
      if(alertBool==byte(0x0)&&isEqual(source,sourceAlert))setMotor(0);
      else{
        setMotor(255);
        for(int i=0;i<8;i++){
          sourceAlert[i]=source[i];
        }
      }
      if(connectionMode)setRGB(r, g, b);
      delay(100);
...
void sendCheck(int row, byte command, byte r, byte g, byte b,byte alert) {
  ...
  xbee.write(b);
  xbee.write(alert);
  byte checksum = 0x10 + command+0x02 + alert + 0x01 + r + g + b + addr[row][0] + addr[row][1] + addr[row][2] + addr[row][3] + addr[row][4] + addr[row][5] + addr[row][6] + addr[row][7] + 0xFF + 0xFE;
  checksum = 0xff - checksum;
  xbee.write(checksum);
  Serial.println(checksum);
}
...
boolean isEqual(byte addressOne[8],byte addressTwo[8]) {
  bool equalRow = true;
  for (int i = 0; i < 8; i++) {
    if (addressOne[i] != addressTwo[i])equalRow = false;
  }
  return equalRow;
}
